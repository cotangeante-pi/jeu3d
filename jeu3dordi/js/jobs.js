const Jobs = {
  _ZONES: {
    ...
    hotel:           { xMin: -46, xMax: -38, zMin: 24, zMax: 32 },
    football:        { xMin: -74, xMax: -66, zMin: 28, zMax: 36 },
    farm:            { xMin: -46, xMax: -38, zMin: 70, zMax: 78 },
    // Pompier : 3 casernes — tableau de zones supporté par _inAnyZone()
    pompier: [
      { xMin: -32, xMax: -24, zMin:   0, zMax:   8 },
      { xMin:  24, xMax:  32, zMin: -78, zMax: -70 },
      { xMin: -46, xMax: -38, zMin: -42, zMax: -34 },
    ],
  },
  ...
  tick(delta) {
    ...
    if (State.salaryTimer >= CONFIG.SALARY_INTERVAL) {
      ...
      this._trackProgress(State.currentJob.id, State.currentJob.salary);
      Sound.coin();
      HUD.update();
      Save.write();
  ...
  earnFromWork(amount) {
    Sound.coin();
    State.money += amount;
    if (State.currentJob) this._trackProgress(State.currentJob.id, amount);
    HUD.update();
    Save.write();
    this._notify(`✓ Bien joué ! +${amount}$`, '#88ff88');
  },

  _trackProgress(id, amount) {
    const key = 'jeu3d_jobs_progress_ordi';
    let data = {};
    try { data = JSON.parse(localStorage.getItem(key) || '{}'); } catch(e) {}
    if (!data[id]) data[id] = { totalEarned: 0 };
    data[id].totalEarned += amount;
