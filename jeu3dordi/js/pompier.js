...
const Pompier = {
  ...
  enter() {
    ...
    this._ePrev = false; this._actionFill = 0; this._rescueProgress = 0;
    this._fireTime = 0;
  ...
  _tickSauvetage(delta) {
    ...
    if (this._victimDist < 3 && this._eHeld) {
      this._rescueProgress = Math.min(1, this._rescueProgress + delta / 1.5);
      this._actionFill = this._rescueProgress;
      if (this._rescueProgress >= 1) {
  ...
  _updateHUD(delta) {
    ...
    if (m === 'incendie') {
      ...
      if (fEl) fEl.style.width = (this._victim?.done ? 100 : this._actionFill * 100) + '%';
      if (pEl) pEl.textContent = 'Progression';
