...
### Contrôles — monde principal
| Touche | Action |
|--------|--------|
| `W A S D` | Se déplacer |
| `Shift` | Sprint |
| Souris | Regarder / orienter la caméra |
| `Espace` | Sauter (maintenu contre un mur = escalader) |
| `E` | Interagir (bâtiment, NPC, ramasser) |
| `T` | Entrer / quitter un poste de travail |
| Clic droit | Ramasser un objet / manger l'item sélectionné |
| Clic gauche | Frapper |
| `1`–`8` | Changer de slot dans la hotbar |
| `F` | Monter / descendre d'une voiture |
| `Z` | Afficher / masquer la position (X, Z) |
| `Échap` | Pause |

### Contrôles — voiture
| Touche | Action |
|--------|--------|
| `W A S D` | Accélérer / freiner / tourner |
| `Shift` | Boost |
...
| 🚴 **Coach** | Équilibre sur monocycle — maintenir `A`/`D` dans la zone verte |
...
| 🚒 **Pompier** | Mini-jeu pompier 3D (3 casernes en ville) |
...
Tu peux devenir propriétaire d'une maison en ville. Une fois acquise, approche-toi de la porte et appuie sur `E` pour l'ouvrir ou la fermer — la porte s'anime à l'ouverture et à la fermeture. Tu peux entrer et sortir librement, et ta maison te sert de refuge.

---

## Monde des Jeux de Société

Au cœur de la ville se dresse une structure monumentale, impossible à ignorer — elle interpelle le regard dès qu'on passe à proximité. Approche-toi et appuie sur `E` pour y entrer : le jeu 3D se met automatiquement en pause et sauvegarde ta progression.
...
└── js/
    ...
    ├── input.js            # Clavier, souris, pointer lock
