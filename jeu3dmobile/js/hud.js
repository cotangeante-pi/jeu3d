const HUD = {
  ...
  update() {
    ...
    } else if (State.nearHouseDoor) {
      hint.style.display = 'block';
      hint.textContent = State.houseDoorOpen ? 'E — Fermer la porte' : 'E — Ouvrir la porte';
    } else if (State.inLightVehicle) {
      const lv = LightVehicles.getActiveVehicle();
      hint.style.display = 'block';
      hint.textContent = lv ? `${lv.icon} ${lv.name}  |  Bouton Voiture — Descendre` : 'Bouton Voiture — Descendre';
    } else {
    ...
    if (coordsEl && coordsEl.style.display !== 'none') {
      coordsEl.textContent = `X: ${State.posX.toFixed(1)}  Z: ${State.posZ.toFixed(1)}`;
    }
  ...
  _badgeName(id) {
    const names = {
      ...
      farm:             '🌾 Fermier',
      lv_bike:          '🚲 Vélo',
      lv_scooter:       '🛴 Trottinette',
      lv_skateboard:    '🛹 Skateboard',
      lv_skates:        '⛸ Patins',
      house:            '🏠 Propriétaire',
